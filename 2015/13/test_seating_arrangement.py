...
Happiness = MutableMapping[str, MutableMapping[str, int]]


def seating_arrangement(happiness: Happiness) -> int:
    all_names = happiness.keys()

    return max(get_happiness_change(permutation, happiness) for permutation in permutations(all_names))


def get_happiness_change(ordered_names: tuple[str, ...], happiness: Happiness) -> int:
    return sum(
        happiness[name][ordered_names[index - 1]] + happiness[name][ordered_names[(index + 1) % len(ordered_names)]]
        for index, name in enumerate(ordered_names)
    )


def parse_input(raw_input: str) -> Happiness:
    output: Happiness = defaultdict(lambda: defaultdict(int))
    ...
    for line in raw_input.splitlines():
        ...
        sign = tokens[2]
        number = int(tokens[3])
        second_name = tokens[10][:-1]
        output[first_name][second_name] = number * (1 if sign == "gain" else -1)
...
PUZZLE_INPUT = """Alice would lose 57 happiness units by sitting next to Bob.
...
@mark.parametrize(("raw_input", "expected_output"), [(EXAMPLE_INPUT, 330), (PUZZLE_INPUT, 618)])
def test_seating_arrangement(raw_input: str, expected_output: int) -> None:
    happiness = parse_input(raw_input)
    assert seating_arrangement(happiness) == expected_output


@mark.parametrize(("raw_input", "expected_output"), [(EXAMPLE_INPUT, 286), (PUZZLE_INPUT, 601)])
def test_seating_arrangement_with_yourself(raw_input: str, expected_output: int) -> None:
    happiness = parse_input(raw_input)
    all_names = set(happiness.keys())

    for name in all_names:
        happiness["Yourself"][name] = 0
        happiness[name]["Yourself"] = 0
