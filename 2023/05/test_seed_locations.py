...
def single_seed_locations(input_string: str) -> int:
    seeds, maps = _parse_input(input_string)
    return min(_evaluate_seed(seed, maps) for seed in seeds)


def seed_range_locations(input_string: str) -> int:
    seeds, maps = _parse_input(input_string)
    seed_ranges = _construct_seed_ranges(seeds)
    evaluated = evaluate_seed_ranges_with_maps(seed_ranges, maps)
    return evaluated[0].start


def _construct_seed_ranges(raw_nums: list[int]) -> list[SeedRange]:
    output = list[SeedRange]()

    for i in range(0, len(raw_nums), 2):
        output.append(SeedRange(start=raw_nums[i], length=raw_nums[i + 1]))

    return output


def _evaluate_seed(seed: int, maps: list[list[RangeMap]]) -> int:
    value = seed

    for current_map in maps:
        potential_map_index = bisect_left(current_map, value, key=lambda r: r.source_start)

        if potential_map_index < len(current_map):
            potential_range = current_map[potential_map_index]
            if (
                value >= potential_range.source_start
                and value < potential_range.source_start + potential_range.range_length
            ):
                value += potential_range.dest_start - potential_range.source_start
                continue

        if potential_map_index - 1 >= 0:
            potential_range = current_map[potential_map_index - 1]
            if (
                value >= potential_range.source_start
                and value < potential_range.source_start + potential_range.range_length
            ):
...
def _parse_input(input_string: str) -> tuple[list[int], list[list[RangeMap]]]:
    lines = input_string.splitlines()
    return _parse_seeds(lines[0]), _parse_maps(lines[2:])


def _parse_maps(lines: list[str]) -> list[list[RangeMap]]:
    output = list[list[RangeMap]]()
    ...
    for line in lines:
        ...
        dest_start, source_start, range_length = (int(token) for token in line.split())
...
def _parse_seeds(line: str) -> list[int]:
    colon_loc = line.find(":")
    return [int(token) for token in line[colon_loc + 2 :].split()]
...
PUZZLE_INPUT = """seeds: 1263068588 44436703 1116624626 2393304 2098781025 128251971 2946842531 102775703 2361566863 262106125 221434439 24088025 1368516778 69719147 3326254382 101094138 1576631370 357411492 3713929839 154258863
...
def evaluate_seed_ranges_with_maps(ranges: list[SeedRange], maps: list[list[RangeMap]]) -> list[SeedRange]:
    for current_map in maps:
        ranges = _evaluate_seed_ranges_with_map(ranges, current_map)

    return ranges


def _evaluate_seed_ranges_with_map(ranges: list[SeedRange], current_map: list[RangeMap]) -> list[SeedRange]:
    output = list[SeedRange]()

    for range in ranges:
        output.extend(_evaluate_seed_range_with_map(range, current_map))

    output.sort()
    return merge_ranges(output)


def _evaluate_seed_range_with_map(range: SeedRange, current_map: list[RangeMap]) -> list[SeedRange]:
    output = list[SeedRange]()
    current_index = bisect_left(current_map, range.start, key=lambda r: r.source_start)

    while (
        current_index > 0
        and range.start < current_map[current_index - 1].source_start + current_map[current_index - 1].range_length
    ):
        current_index -= 1

    if current_index == len(current_map):
        return [range]

    while range.length and current_index < len(current_map):
        current_map_item = current_map[current_index]

        if range.start + range.length <= current_map_item.source_start:
            output.append(range)
            break

        if range.start < current_map_item.source_start:
            length = current_map_item.source_start - range.start
            output.append(SeedRange(range.start, length))
            range.start += length
            range.length -= length
        else:
            end = min(current_map_item.source_start + current_map_item.range_length, range.start + range.length)
            length = end - range.start
            output.append(SeedRange(range.start + current_map_item.dest_start - current_map_item.source_start, length))
            range.start += length
            range.length -= length
            current_index += 1
...
def merge_ranges(ranges: list[SeedRange]) -> list[SeedRange]:
    if not ranges:
        return []

    output = list[SeedRange]([ranges[0]])

    for index in range(1, len(ranges)):
        if ranges[index].start > output[-1].start + output[-1].length:
            output.append(ranges[index])
        else:
            output[-1].length = ranges[index].start + ranges[index].length - output[-1].start

    return output


@mark.parametrize(
    ("input_ranges", "expected_output"),
    [
        ([], []),
        ([SeedRange(start=0, length=10)], [SeedRange(start=0, length=10)]),
        ([SeedRange(start=0, length=5), SeedRange(start=5, length=5)], [SeedRange(start=0, length=10)]),
        (
            [SeedRange(start=0, length=5), SeedRange(start=6, length=5)],
            [SeedRange(start=0, length=5), SeedRange(start=6, length=5)],
        ),
        (
            [SeedRange(start=0, length=1), SeedRange(start=1, length=2), SeedRange(start=3, length=3)],
            [SeedRange(start=0, length=6)],
        ),
    ],
)
def test_merge_ranges(input_ranges: list[SeedRange], expected_output: list[SeedRange]) -> None:
    assert merge_ranges(input_ranges) == expected_output


@mark.parametrize(
    ("input_ranges", "maps", "expected_output"),
    [
        ([SeedRange(0, 10)], [[]], [SeedRange(0, 10)]),
        ([SeedRange(5, 10)], [[RangeMap(source_start=10, dest_start=0, range_length=5)]], [SeedRange(0, 10)]),
        (
            [SeedRange(10, 3)],
            [
                [RangeMap(source_start=12, dest_start=0, range_length=1)],
                [RangeMap(source_start=11, dest_start=1, range_length=1)],
                [RangeMap(source_start=10, dest_start=2, range_length=1)],
            ],
            [SeedRange(0, 3)],
        ),
        (
            [SeedRange(0, 4)],
            [[RangeMap(source_start=1, dest_start=5, range_length=2)]],
            [SeedRange(0, 1), SeedRange(3, 1), SeedRange(5, 2)],
        ),
        (
            [SeedRange(55, 13), SeedRange(79, 14)],
            [
                [
                    RangeMap(source_start=50, dest_start=52, range_length=48),
                    RangeMap(source_start=98, dest_start=50, range_length=2),
                ]
            ],
            [SeedRange(57, 13), SeedRange(81, 14)],
        ),
        (
            [SeedRange(start=57, length=13)],
            [
                [
                    RangeMap(source_start=0, dest_start=42, range_length=7),
                    RangeMap(source_start=7, dest_start=57, range_length=4),
                    RangeMap(source_start=11, dest_start=0, range_length=42),
                    RangeMap(source_start=53, dest_start=49, range_length=8),
                ]
            ],
            [SeedRange(start=53, length=4), SeedRange(start=61, length=9)],
        ),
    ],
)
def test_evaluate_seed_ranges_with_maps(
    input_ranges: list[SeedRange], maps: list[list[RangeMap]], expected_output: list[SeedRange]
) -> None:
    assert evaluate_seed_ranges_with_maps(input_ranges, maps) == expected_output


@mark.parametrize(("input_string", "expected_output"), [(EXAMPLE_INPUT, 35), (PUZZLE_INPUT, 199602917)])
def test_single_seed_locations(input_string: str, expected_output: int) -> None:
    assert single_seed_locations(input_string) == expected_output


@mark.parametrize(("input_string", "expected_output"), [(EXAMPLE_INPUT, 46), (PUZZLE_INPUT, 2254686)])
def test_seed_range_locations(input_string: str, expected_output: int) -> None:
    assert seed_range_locations(input_string) == expected_output
