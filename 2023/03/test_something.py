...
def part_number_sum(input_string: str) -> int:
    grid = [list(line) for line in input_string.splitlines()]
    total = 0

    for n, y, x1, x2 in _find_numbers(grid):
        if len(list(_adjacent_stars(grid, y, x1, x2, _is_symbol))):
            total += n

    return total


def gear_ratio_sum(input_string: str) -> int:
    grid = [list(line) for line in input_string.splitlines()]
    star_numbers = defaultdict[tuple[int, int], list[int]](lambda: list[int]())

    for n, y, x1, x2 in _find_numbers(grid):
        for star_x, star_y in _adjacent_stars(grid, y, x1, x2, _is_star):
            star_numbers[(star_x, star_y)].append(n)

    return sum(prod(nums) for nums in star_numbers.values() if len(nums) == 2)


def _find_numbers(grid: list[list[str]]) -> Iterable[tuple[int, int, int, int]]:
        ...
        for x, char in enumerate(row):
            if char.isdigit():
                ...
                if start_x is not None:
                    n = int("".join(row[start_x:x]))
                    yield n, y, start_x, x
                    start_x = None

        if start_x is not None:
            n = int("".join(row[start_x : len(row)]))
            yield n, y, start_x, x
            start_x = None


def _adjacent_stars(
    grid: list[list[str]], y: int, x1: int, x2: int, symbol_checker: Callable[[str], bool]
) -> Iterable[tuple[int, int]]:
    for adjacent_x, adjacent_y in _adjacent_cells(grid, y, x1, x2):
        if symbol_checker(grid[adjacent_y][adjacent_x]):
            yield adjacent_x, adjacent_y


def _adjacent_cells(grid: list[list[str]], y: int, x1: int, x2: int) -> Iterable[tuple[int, int]]:
    if y > 0:
        for x in range(max(x1 - 1, 0), min(x2 + 1, len(grid[0]))):
            yield x, y - 1
...
...$.*....
.664.598.."""
PUZZLE_INPUT = """.....180.........230..........................218.....189......415.......................322....507..................206..............111...
...
@mark.parametrize(("input_string", "expected_output"), [(EXAMPLE_INPUT, 4361), (PUZZLE_INPUT, 537832)])
def test_part_number_sum(input_string: str, expected_output: int) -> None:
    assert part_number_sum(input_string) == expected_output


@mark.parametrize(("input_string", "expected_output"), [(EXAMPLE_INPUT, 467835), (PUZZLE_INPUT, 81939900)])
def test_gear_ratio_sum(input_string: str, expected_output: int) -> None:
    assert gear_ratio_sum(input_string) == expected_output
