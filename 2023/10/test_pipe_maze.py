...
def _compute_distances(maze: list[list[str]], start_loc: Point) -> list[list[int | None]]:
    distances: list[list[int | None]] = [[None for _ in row] for row in maze]
    stack = deque([(start_loc, 0)])

    while stack:
        loc, dist = stack.popleft()

        if distances[loc.y][loc.x] is not None:
            continue

        distances[loc.y][loc.x] = dist
        stack.extend((neighbour, dist + 1) for neighbour in _get_neighbours(loc, maze))

    return distances


def _compute_inside(maze: list[list[str]], start_loc: Point) -> list[list[bool]]:
    inside = []
    distances = _compute_distances(maze, start_loc)

    for y, row in enumerate(maze):
        inside_row = list[bool]()
        edge_count = 0
        ...
        for x, cell in enumerate(row):
            if distances[y][x] is not None:
                if cell == "|":
                    edge_count += 1
                    inside_row.append(False)
                elif cell == "-":
                    inside_row.append(False)
                elif cell == "L":
                    if current_edge_start is None:
                        current_edge_start = "L"
                        inside_row.append(False)
                elif cell == "J":
                    if current_edge_start is None:
                        current_edge_start = "J"
                        inside_row.append(False)
                    elif current_edge_start == "L":
                        edge_count += 0
                        current_edge_start = None
                        inside_row.append(False)
                    elif current_edge_start == "F":
                        edge_count += 1
                        current_edge_start = None
                        inside_row.append(False)
                    else:
                    ...
                    if current_edge_start is None:
                        current_edge_start = "F"
                        inside_row.append(False)
                    else:
                        raise RuntimeError("Unexpected F")
                else:
                    raise RuntimeError(f"Unexpected {cell=}")
            else:
                inside_row.append(edge_count % 2 == 1)
        inside.append(inside_row)

    return inside


def _get_neighbours(loc: Point, maze: list[list[str]]) -> Iterable[Point]:
    curr = maze[loc.y][loc.x]
    offsets = PIPE_MAP.get(curr, [])

    if not offsets:
        return

    for offset in offsets:
        potential = maze[loc.y + offset.y][loc.x + offset.x]
        if any(
            offset.x == -neighbour_offset.x and offset.y == -neighbour_offset.y
            for neighbour_offset in PIPE_MAP.get(potential, [])
        ):
            yield Point(loc.x + offset.x, loc.y + offset.y)
...
EXAMPLE_INPUT5 = """FF7F7F7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
PUZZLE_INPUT = """7.LJFFF7F77.----7F7777F|-F-7-7F77LL77J77F|.|JFF7-F|77.FF77|F77F-L|-L7FFL7L7--.L|--7L7.J-F7-|-F|7FJ.FJJ7F7-7-7JFL|7-|-7FJJ7FFF7LL--L7.-JJ-F-F
...
|.|F7FL|-F--JF7L-7||F-JF7L7L7|FJ|LL7L-7F7F-7L-JL--77LJF7F7.|LJ7J7|-JFLL--7F7|L--JFJL7L7||F-7|F7F7L---7|L----JF7F---JF7FJF7L------JL-7J|F-7-7
...
@mark.parametrize(
    ("input_string", "start_loc", "expected_output"),
    [(EXAMPLE_INPUT1, Point(1, 1), 4), (EXAMPLE_INPUT2, Point(0, 2), 8), (PUZZLE_INPUT, Point(24, 76), 6842)],
)
def test_pipe_maze_furthest_distance(input_string: str, start_loc: Point, expected_output: int) -> None:
    assert pipe_maze_furthest_distance(input_string, start_loc) == expected_output


@mark.parametrize(
    ("input_string", "start_loc", "expected_output"),
    [
        (EXAMPLE_INPUT3, Point(1, 1), 4),
        (EXAMPLE_INPUT4, Point(1, 1), 4),
        (EXAMPLE_INPUT5, Point(0, 4), 10),
        (PUZZLE_INPUT, Point(24, 76), 393),
    ],
)
def test_pipe_maze_inside_loop(input_string: str, start_loc: Point, expected_output: int) -> None:
    assert pipe_maze_inside_loop(input_string, start_loc) == expected_output
